			...
			<div class="mt-2 flex items-center justify-between text-sm text-neutral-400 dark:text-neutral-500">
				{props.text().length > 0 ? (
					<button
						type="button"
						onClick={handleClear}
						class="px-2.5 py-1 text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 transition-colors"
					>
						Clear
					</button>
				) : (
				...
				<span class="tabular-nums">
					{props.text().length.toLocaleString()} characters
				</span>
			</div>
		</div>
	);
